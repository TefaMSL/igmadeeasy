/* =============================================
   VIION GROUP — Ultra Premium Scripts v2
   ============================================= */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (chatToggle && chatWindow) {
    ...
    let messageCount = 0;
    const botReplies = [
      "I see. Our infrastructure team specializes in exactly this for healthcare facilities.",
      "Could you clarify how many clinics or branches are currently in your network?",
      "That makes sense. We can definitely streamline that process and ensure 100% data compliance.",
      "Would you like me to schedule a direct call with Eng. Ahmed Mahmoud to discuss this further?"
    ];

    const specificReplies = {
      "EHR/EMR Support": "We have extensive experience integrating and supporting leading EHR/EMR systems to ensure seamless clinical workflows and GAHAR compliance. How many branches are in your network?",
      "IT Infrastructure": "Our IT infrastructure solutions are designed specifically for the high availability and security demands of modern healthcare facilities. Are you looking to upgrade an existing setup?",
      "Cloud Migration": "We provide secure, HIPAA-compliant cloud migration services tailored for hospitals, ensuring zero downtime and scalable storage. Do you have a preferred cloud provider?",
      "Cybersecurity": "Healthcare data is highly sensitive. We implement robust cybersecurity protocols, including endpoint protection and encrypted networks. Would you like a free security assessment?"
    };
    ...
    const sendUserMessage = (text) => {
      ...
      setTimeout(() => {
        ...
        if (specificReplies[text]) {
           ...
           // Remove from specific replies so it doesn't repeat if they type it again,
           // and let the normal conversation flow take over next time.
           delete specificReplies[text];
        } else if (messageCount < botReplies.length) {
           botMsg.textContent = botReplies[messageCount];
           messageCount++;
        } else {
           botMsg.textContent = "Thank you! I have forwarded these details to our team. We'll be in touch soon.";
        }
        
        chatBody.appendChild(botMsg);
        
        if (text === "Main Menu") {
          appendOptions(["EHR/EMR Support", "IT Infrastructure", "Cloud Migration", "Cybersecurity"]);
        } else {
          appendOptions(["Main Menu", "Talk to Sales"]);
        }
  ...
  if (canvas) {
    ...
    window.addEventListener('mousemove', e => {
      ...
      if(orbs.length) {
        ...
        orbs.forEach((orb, index) => {
          const factor = (index + 1) * -1.2; // Move opposite to mouse
    ...
    class Node {
      constructor() {
        ...
        // Vary hue between cyan (199), electric blue (220) and deep blue (240)
  ...
  async function fetchPortfolio() {
    ...
    try {
      ...
      data.forEach((proj, index) => {
        ...
        // Handle custom theme color from CMS
        const colorVar = `var(--${proj.theme_color || 'cyan'})`;
        block.style.setProperty('--card-glow', colorVar);
        
        // Apply glowing left border using a dynamic style trick, or rely on CSS nth-child if limited to 4
        
        let bulletsHtml = proj.bullets_en.map(b => `
          <li>
        ...
        block.innerHTML = `
          <span class="case-tag" style="background: rgba(var(--${proj.theme_color || 'cyan'}-rgb, 56,189,248), 0.1); color: ${colorVar}">${proj.tag_en}</span>
          <h3>${proj.title_en}</h3>
          <ul class="case-list">
            ${bulletsHtml}
          </ul>
